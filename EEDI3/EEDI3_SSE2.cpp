...
void filter_sse2(const VSFrameRef * src, const VSFrameRef * scp, const VSFrameRef * mclip, VSFrameRef * mcp, VSFrameRef * dst, VSFrameRef ** pad,
                ...
                for (int x = 1; x < dstWidth; x++) {
                    ...
                    if (bmask && !bmask[x]) {
                        if (x == 1) {
                            ...
                            if (pumax < d->mdis) {
                                Vec4i(1 - pumax).stream(piT - pumax * d->vectorSize);
                                Vec4i(pumax - 1).stream(piT + pumax * d->vectorSize);
                            }
                        }
                    } else {
                        const int umax = std::min({ x, dstWidth - 1 - x, d->mdis });
...
template void filter_sse2<uint8_t, int>(const VSFrameRef *, const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3Data * const VS_RESTRICT, const VSAPI *) noexcept;
template void filter_sse2<uint16_t, int>(const VSFrameRef *, const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3Data * const VS_RESTRICT, const VSAPI *) noexcept;
template void filter_sse2<float, float>(const VSFrameRef *, const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3Data * const VS_RESTRICT, const VSAPI *) noexcept;
#endif
