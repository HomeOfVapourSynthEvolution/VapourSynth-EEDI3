...
static inline void calculateConnectionCosts(const void * srcp, float * ccosts, const int width, const int stride, const EEDI3Data * d) noexcept {
    const __m256i * src3p = reinterpret_cast<const __m256i *>(srcp) + 12;
    const __m256i * src1p = src3p + stride;
    const __m256i * src1n = src1p + stride;
    const __m256i * src3n = src1n + stride;
    ...
    if (d->cost3) {
        for (int x = 0; x < width; x++) {
            ...
            for (int u = -umax; u <= umax; u++) {
                ...
                const Vec8f v = to_float(abs(Vec8i().load_a(src1p + x) - ip) + abs(Vec8i().load_a(src1n + x) - ip));
                const Vec8f result = mul_add(d->alpha, to_float(s0 + s1 + s2), d->beta * std::abs(u)) + d->remainingWeight * v;
        ...
        for (int x = 0; x < width; x++) {
            ...
            for (int u = -umax; u <= umax; u++) {
                ...
                const Vec8i ip = (Vec8i().load_a(src1p + x + u) + Vec8i().load_a(src1n + x - u) + 1) >> 1; // should use cubic if ucubic=true
                const Vec8f v = to_float(abs(Vec8i().load_a(src1p + x) - ip) + abs(Vec8i().load_a(src1n + x) - ip));
                const Vec8f result = mul_add(d->alpha, to_float(s), d->beta * std::abs(u)) + d->remainingWeight * v;
                result.stream(ccosts + (d->tpitch * x + u) * d->vectorSize);
            }
        }
    }
...
template void process_avx2<uint8_t, int>(const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3Data *, const VSAPI *) noexcept;
template void process_avx2<uint16_t, int>(const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3Data *, const VSAPI *) noexcept;
template void process_avx2<float, float>(const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3Data *, const VSAPI *) noexcept;
#endif
