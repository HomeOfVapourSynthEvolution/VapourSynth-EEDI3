...
template<typename T>
void processCL_avx2(const VSFrameRef * src, const VSFrameRef * scp, VSFrameRef * dst, VSFrameRef ** pad, const int field_n, const EEDI3CLData * d, const VSAPI * vsapi) {
    for (int plane = 0; plane < d->vi.format->numPlanes; plane++) {
        if (d->process[plane]) {
            ...
            for (int y = 4 + field_n; y < srcHeight - 4; y += 2 * d->vectorSize) {
                ...
                Vec8f().load(ccosts).store_a(pcosts);
                for (int x = 1; x < dstWidth; x++) {
                    ...
                    for (int u = -umax; u <= umax; u++) {
                        ...
                        const Vec8f z = bval + Vec8f().load(tT + u * d->vectorSize);
...
template void processCL_avx2<uint8_t>(const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3CLData *, const VSAPI *);
template void processCL_avx2<uint16_t>(const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3CLData *, const VSAPI *);
template void processCL_avx2<float>(const VSFrameRef *, const VSFrameRef *, VSFrameRef *, VSFrameRef **, const int, const EEDI3CLData *, const VSAPI *);
#endif
