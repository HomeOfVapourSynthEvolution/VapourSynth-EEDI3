        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16f result = mul_add(d->alpha, to_float(s0 + s1 + s2), mul_add(d->remainingWeight, to_float(v), d->beta * std::abs(u)));
        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16i ip = (Vec16i().load_a(src1p + x + u) + Vec16i().load_a(src1n + x - u) + 1) >> 1; // should use cubic if ucubic=true
                    const Vec16i v = abs(Vec16i().load_a(src1p + x) - ip) + abs(Vec16i().load_a(src1n + x) - ip);
                    const Vec16f result = mul_add(d->alpha, to_float(s), mul_add(d->remainingWeight, to_float(v), d->beta * std::abs(u)));
                    result.store_nt(ccosts + (d->tpitch * x + d->mdis + u) * d->vectorSize);
                }
            }
        }
        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16f result = mul_add(d->alpha, s0 + s1 + s2, mul_add(d->remainingWeight, v, d->beta * std::abs(u)));
                    result.store_nt(ccosts + (d->tpitch * x + d->mdis + u) * d->vectorSize);
                }
            }
        }
        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16f ip = (Vec16f().load_a(src1p + x + u) + Vec16f().load_a(src1n + x - u)) * 0.5f; // should use cubic if ucubic=true
                    const Vec16f v = abs(Vec16f().load_a(src1p + x) - ip) + abs(Vec16f().load_a(src1n + x) - ip);
                    const Vec16f result = mul_add(d->alpha, s, mul_add(d->remainingWeight, v, d->beta * std::abs(u)));
                    result.store_nt(ccosts + (d->tpitch * x + d->mdis + u) * d->vectorSize);
                }
            }
        }
                    ...
                    if (bmask && !bmask[x]) {
                        ...
                        for (int u = -umax; u <= umax; u++) {
                            ...
                            for (int v = std::max(-umax2, u - 1); v <= std::min(umax2, u + 1); v++) {
                                const Vec16f z = Vec16f().load_a(ppT + (d->mdis + v) * d->vectorSize) + d->gamma * std::abs(u - v);
