...
static inline void calculateConnectionCosts(const vector_t* srcp, const bool* bmask, float* ccosts, const int width, const int stride,
    ...
    auto src3p = reinterpret_cast<const __m512i*>(srcp) + MARGIN_H;
        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16i v = abs(Vec16i().load_a(src1p + x) - ip) + abs(Vec16i().load_a(src1n + x) - ip);
                    const Vec16f result = mul_add(d->alpha, to_float(s0 + s1 + s2), mul_add(d->beta, std::abs(u), d->remainingWeight * to_float(v)));
        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16i ip = (Vec16i().load_a(src1p + x + u) + Vec16i().load_a(src1n + x - u) + 1) >> 1; // should use cubic if ucubic=true
                    const Vec16i v = abs(Vec16i().load_a(src1p + x) - ip) + abs(Vec16i().load_a(src1n + x) - ip);
                    const Vec16f result = mul_add(d->alpha, to_float(s), mul_add(d->beta, std::abs(u), d->remainingWeight * to_float(v)));
                    result.store_nt(ccosts + (d->tpitch * x + d->mdis + u) * d->vectorSize);
                }
            }
        }
...
inline void calculateConnectionCosts(const float* srcp, const bool* bmask, float* ccosts, const int width, const int stride,
                                     const EEDI3Data* VS_RESTRICT d) noexcept {
    auto src3p = reinterpret_cast<const __m512*>(srcp) + MARGIN_H;
    auto src1p = src3p + stride;
    auto src1n = src1p + stride;
    auto src3n = src1n + stride;
        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16f result = mul_add(d->alpha, s0 + s1 + s2, mul_add(d->beta, std::abs(u), d->remainingWeight * v));
                    result.store_nt(ccosts + (d->tpitch * x + d->mdis + u) * d->vectorSize);
                }
            }
        }
        ...
        for (int x = 0; x < width; x++) {
            if (!bmask || bmask[x]) {
                ...
                for (int u = -umax; u <= umax; u++) {
                    ...
                    const Vec16f ip = (Vec16f().load_a(src1p + x + u) + Vec16f().load_a(src1n + x - u)) * 0.5f; // should use cubic if ucubic=true
                    const Vec16f v = abs(Vec16f().load_a(src1p + x) - ip) + abs(Vec16f().load_a(src1n + x) - ip);
                    const Vec16f result = mul_add(d->alpha, s, mul_add(d->beta, std::abs(u), d->remainingWeight * v));
                    result.store_nt(ccosts + (d->tpitch * x + d->mdis + u) * d->vectorSize);
                }
            }
        }
...
void filter_avx512(const VSFrame* src, const VSFrame* scp, const VSFrame* mclip, VSFrame* mcp, VSFrame** pad, VSFrame* dst, void* _srcVector,
                   uint8_t* _mskVector, bool* VS_RESTRICT bmask, float* ccosts, float* pcosts, int* _pbackt, int* VS_RESTRICT fpath, int* _dmap,
            ...
            for (int y = field_n; y < dstHeight; y += 2 * d->vectorSize) {
                ...
                if (bmask) {
                    ...
                    for (int x = 0; x < minmdis; x++)
                        if (mskVector[x * 2] != 0 || mskVector[x * 2 + 1] != 0)
                            last = x + d->mdis;

                    for (int x = 0; x < dstWidth - minmdis; x++) {
                        if (mskVector[(x + d->mdis) * 2] != 0 || mskVector[(x + d->mdis) * 2 + 1] != 0)
                            last = x + d->mdis * 2;
                ...
                Vec16f().load_a(ccosts + d->mdisVector).store_a(pcosts + d->mdisVector);
                for (int x = 1; x < dstWidth; x++) {
                    ...
                    if (bmask && !bmask[x]) {
                        if (x == 1) {
                            ...
                            if (pumax < d->mdis) {
                                Vec16i(1 - pumax).store_nt(piT + (d->mdis - pumax) * d->vectorSize);
                                Vec16i(pumax - 1).store_nt(piT + (d->mdis + pumax) * d->vectorSize);
                            }
                        }
                    } else {
                        const int umax = std::min({ x, dstWidth - 1 - x, d->mdis });
                        ...
                        for (int u = -umax; u <= umax; u++) {
                            Vec16i idx = zero_si512();
                            Vec16f bval = FLT_MAX;

                            for (int v = std::max(-umax2, u - 1); v <= std::min(umax2, u + 1); v++) {
                                const Vec16f z = mul_add(d->gamma, std::abs(u - v), Vec16f().load_a(ppT + (d->mdis + v) * d->vectorSize));
                            ...
                            const Vec16f z = bval + Vec16f().load_a(tT + mu);
...
template void filter_avx512<uint8_t, int>(const VSFrame* src, const VSFrame* scp, const VSFrame* mclip, VSFrame* mcp, VSFrame** pad, VSFrame* dst,
...
template void filter_avx512<uint16_t, int>(const VSFrame* src, const VSFrame* scp, const VSFrame* mclip, VSFrame* mcp, VSFrame** pad, VSFrame* dst,
                                           void* _srcVector, uint8_t* _mskVector, bool* VS_RESTRICT bmask, float* ccosts, float* pcosts, int* _pbackt,
                                           int* VS_RESTRICT fpath, int* _dmap, const int field_n, const EEDI3Data* VS_RESTRICT d, const VSAPI* vsapi) noexcept;

template void filter_avx512<float, float>(const VSFrame* src, const VSFrame* scp, const VSFrame* mclip, VSFrame* mcp, VSFrame** pad, VSFrame* dst,
                                          void* _srcVector, uint8_t* _mskVector, bool* VS_RESTRICT bmask, float* ccosts, float* pcosts, int* _pbackt,
                                          int* VS_RESTRICT fpath, int* _dmap, const int field_n, const EEDI3Data* VS_RESTRICT d, const VSAPI* vsapi) noexcept;
#endif
